...
def group_domains(domains):
    ...
    for domain in domains:
        s = domain.split('.')
        sub_domain = '.'.join(s[0:-1])
        root_domain = s[-1]

        if root_domain is '':
            continue

        if r.has_key(root_domain):
            r[root_domain] = r[root_domain] + '|' + sub_domain
        else:
            r[root_domain] = sub_domain
...
def generate_pac(domains, proxy):
    # render the pac file
    proxy_content = pkgutil.get_data('gfwlist2pac', 'resources/proxy.min.pac')
    domains_dict = group_domains(domains)
    proxy_content = proxy_content.replace('__PROXY__', json.dumps(str(proxy)))
    proxy_content = proxy_content.replace('__DOMAINS__', json.dumps(domains_dict, separators=(',',':')))
